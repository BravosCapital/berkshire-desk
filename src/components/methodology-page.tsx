...
export function MethodologyPage() {
  useEffect(() => {
    document.title = "Methodology · Berkshire Desk";
  }, []);
  ...
  return (
    <AppShell>
      <main className="mx-auto max-w-3xl space-y-8 px-4 py-6 sm:px-6 sm:py-10">
        ...
        <section className="space-y-3 text-sm leading-relaxed text-muted">
          <h2 className="font-display text-xl font-medium text-fg">Analyst tools</h2>
          <p>
            The Overview page includes a set of live scenario tools that sit on top of the same
            two-column identity. They do not change the published IV; they answer “what if”
            questions an analyst would ask at a desk.
          </p>
          <ul className="list-disc space-y-2 pl-5">
            <li>
              <span className="text-fg">Implied operating multiple</span> — backs out the pretax
              multiple the market is paying for non-insurance ops after stripping cash, public
              equities, other investments and the insurance franchise, then adding back parent
              bonds. Useful for comparing the desk multiple to the residual the shares imply.
            </li>
            <li>
              <span className="text-fg">Look-through earnings</span> — Berkshire’s share of TTM net
              income of the public portfolio (where available) plus after-tax ops run-rate plus
              after-tax underwriting. Gives a rough owner-earnings number that can be compared with
              the capitalized value.
            </li>
            <li>
              <span className="text-fg">Cash deployment</span> — sliders that retire B-equivalent
              shares at the live BRK.B price or swap cash for additional public equities. Shows
              cash left, shares retired, and the resulting IV / B and premium. Persists in the
              browser.
            </li>
            <li>
              <span className="text-fg">Earnings × multiple sensitivity</span> — a small grid that
              shocks the pretax run-rate ±10/20 % and re-capitalizes at 12× / 15× / 18×. The same
              grid appears on the Charts page for a fuller view.
            </li>
          </ul>
          <p>
            Daily local snapshots of IV and price are also recorded in the browser so the history
            chart can show intra-quarter moves without a server store.
          </p>
        </section>
